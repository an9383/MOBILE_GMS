...
{
    public class Product_StackStandbyService
    {
        static public ILogger<Controller> _logger;
        public BllSpExecute _bllSpExecute = new BllSpExecute();

        public Product_StackStandbyService(ILogger<Controller> logger)
        {
            _logger = logger;
        }

        public List<Product_StackStandbyModel> Product_StackStandbySearch(Product_StackStandbyModel model)
        {

            List<Product_StackStandbyModel> list = new List<Product_StackStandbyModel>();

            string SP_name = "SP_PDA_Product_StackStandby"; //  procedure 명 
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                cm.pack_size = row["pack_size"].ToString();
                list.Add(cm);
            }
            return list;
        }
