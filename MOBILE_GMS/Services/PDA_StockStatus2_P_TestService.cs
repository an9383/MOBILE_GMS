...
{
    public class PDA_StockStatus2_P_TestService : Controller
    {
        static public ILogger<Controller> _logger;
        public BllSpExecute _bllSpExecute = new BllSpExecute();

        public PDA_StockStatus2_P_TestService(ILogger<Controller> logger)
        {
            _logger = logger;
        }
        ...
        {
            _logger.LogInformation(" [ logger ] :  PDA_StockStatus2_P_TestService > StackStandbySearch() Access : " + DateTime.Now);

            List<StockStatusModel> list = new List<StockStatusModel>();

            string SP_name = "SP_StockStatus2";
            string gubun = "Select1";
